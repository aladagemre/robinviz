...
**/
void drawParallelCoordinate( matrix M, array<GENES> &genes, array<CONDS> &conds, char filename[ 64 ] ){
	FILE *fptr,*fptr2;
	char c;
	fptr = fopen( filename, "w" );
#ifdef LINUX
	fptr2 = fopen( "src//cpp//incl/othervis/pc_start.txt", "r" );
#else
	fptr2 = fopen( "src//cpp//incl//othervis//pc_start.txt", "r" );
#endif
	while( !feof( fptr2 ) ){
	...
	for( int i = 0; i < M.dim1(); i++ ){
		for( int j = 0; j < M.dim2(); j++ ){
			if( j == -1 ){
				...
				fprintf( fptr, "data.setCell(%d, %d, %lf); ", i, j, M( i, j ) );
			}
		}
		fprintf( fptr, "\n" );
	}
// 	cout << "DONE\n";
#ifdef LINUX
	fptr2 = fopen( "src/cpp/incl/othervis/pc_end.txt", "r" );
#else
	fptr2 = fopen( "src//cpp//incl//othervis//pc_end.txt", "r" );
#endif
	while( !feof( fptr2 ) ){
...
* @param filename(char *) output folder
**/
void drawParallelCoordinate( matrix M, list<GENES> &genes, list<CONDS> &conds, char filename[ 64 ] ){
	FILE *fptr,*fptr2;
	char c;
	fptr = fopen( filename, "w" );
#ifdef LINUX
